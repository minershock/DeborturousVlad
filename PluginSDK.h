...
#define PluginSDKSetup(x) \
	gpSDK				= x; \
	gpOrbwalker			= x->GetOrbwalking(); \
	gpEventManager		= x->GetEventManager(); \
	gpRender			= x->GetRenderer(); \
	gpEntityList		= x->GetEntityList(); \
	gpGame				= x->GetGame(); \
	gpDamage			= x->GetDamage(); \
	gpPrediction		= x->GetPrediction(); \
	gpHealthPrediction	= x->GetHealthPrediction(); \
	gpTargetSelector	= x->CreateTargetSelector(); \
	gpSpellData			= x->GetSpellDataReader();
...
public:
	...
	virtual void FindBestCastPosition(float Range, float Radius, bool IsMissile, bool CheckMinions, bool CheckHeroes, Vec3& CastPosition, int& Enemies) = 0;
	...
	virtual void FindBestCastPosition(float Range, float Radius, bool IsMissile, bool CheckMinions, bool CheckHeroes, Vec3& CastPosition, int& Enemies, float Delay) = 0;
};
...
public:
	...
	virtual Vec3 const& GetStartPosition(void* Data) = 0;
	virtual Vec3 const& GetEndPosition(void* Data) = 0;
...
extern IPluginSDK*			gpSDK;
extern IEventManager*		gpEventManager;
extern IRender*				gpRender;
extern IEntityList*			gpEntityList;
extern IGame*				gpGame;
extern IDamage*				gpDamage;
extern IPrediction*			gpPrediction;
extern IHealthPrediction*	gpHealthPrediction;
extern ITargetSelector*		gpTargetSelector;
extern IOrbwalking*			gpOrbwalker;
extern ISpellData*			gpSpellData;
